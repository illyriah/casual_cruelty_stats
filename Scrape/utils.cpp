...
void XLR_Parser::set_failure(std::string custom_message)
{
	errorBuffer = custom_message + errorBuffer;
	failure = true;
}

XLR_Parser::UrT_stats XLR_Parser::get_failure()
{
...
xmlNode* XLR_Parser::get_next_tag(xmlNode*& current_tag, const std::string& tag_name)
{
	xmlNode * node_cursor;
...
xmlXPathObjectPtr XLR_Parser::get_xpath_nodes(xmlDocPtr document_pointer, const std::string& xpath_expression, xmlNode * new_root)
{

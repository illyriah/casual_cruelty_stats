...
-- Returns join stats by map for all non-RSM players.
...
INNER JOIN (SELECT map.map_name, count(player_connection.player_connection_id) as departcount FROM match 
	...
	INNER JOIN player ON player.player_id = player_connection.player_id
	WHERE player.is_rsm = '0' AND connection_end_time > match_start_time AND connection_end_time <= match_end_time
...
INNER JOIN (SELECT map.map_name, count(player_connection.player_connection_id) as joincount FROM match 
	...
	INNER JOIN player_connection ON player_connection.match_id = match.match_id
	INNER JOIN player ON player.player_id = player_connection.player_id
	WHERE player.is_rsm = '0' AND connection_start_time >= match_start_time AND connection_start_time < match_end_time
